...
(function (global, cc) {
	...
	var commonRes = {
		...
		roles: [
			rootPath + '/ui/role/role1.png',
			rootPath + '/ui/role/role2.png',
			rootPath + '/ui/role/role3.png',
			rootPath + '/ui/role/role4.png',
			rootPath + '/ui/role/role5.png',
			rootPath + '/ui/role/role6.png'
		]
	};
...
(function (wnd) {
	var GameStartLayer = cc.Layer.extend({
		ctor: function (data, callback, target) {
			...
			// cc.audioPlayer.playAudio(config.effect.bgm, true, 0.2);
	...
	var GameOverLayer = cc.Layer.extend({
		...
		makeFlowers: function () {
			var h = cc.winSize.height;
			var posx = [600, 610, 620, 630, 640, 650, 660, 670];
			...
			var pos = [600, 610, 590, 580, 680, 690, 660, 670];
			for(var j=0;j<8;j++){
		...
		initRole: function () {
			...
			var animation = new AnimationFrames(emotion);
	...
	var Flower = cc.Sprite.extend({
		...
		disapper: function () {
			var endPos = cc.p(cc.winSize.width - 100, cc.winSize.height - 50);

			var points = [cc.p(this.x, this.y), cc.p(1200, 300), endPos];
